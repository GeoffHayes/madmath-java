...
public class ObservationGenerationRunnable implements Runnable {
    ...
    public ObservationGenerationRunnable(
            Context context, final SelfTrackerActivity.AppConfigData appConfigData, Handler handler)
    {
    ...
    public void run()
    {
        ...
        _obs2DP.init((double)currentTimeMSecs/SEC2MSEC);
        ...
        if ((_lastKnownLocation != null && _isLocationUpdateWaiting) || _inDemoMode)
        {
            ...
            {        
                ...
                try
                {
                    ...
                    _obs2DP.setInitTime((double)currentTimeMSecs/SEC2MSEC);
                    ...
                    {
                        posUncertainty = (double)_lastKnownLocation.getAccuracy();
                    }
                    else
                    {
        ...
        try
        {
            if (_inDemoMode)
            {
                try
                {
                    ...
                    File pathAndFile = new File(path,DATA_FILE);
